...
export const constructPsbt = async (
  ...
  // Get UTXOs
  const utxos = await getBalance({ address, getUtxos: true }, networkId);
  ...
  await Promise.all(
    utxos.map(async (utxo: Utxo) => {
      ...
      const transaction = await fetchTransaction(utxo.txid, networkId);
      let inputOptions;
      ...
      if (scriptHex.startsWith("76a914")) {
        ...
        inputOptions = {
          ...
          tapInternalKey: Buffer.from("taprootInternalPubKey", "hex"), // Convert to Buffer
        };
      } else {
...
export const bitcoin = {
  ...
  getBalance: async (
    { address, getUtxos = false }: { address: string; getUtxos?: boolean },
    networkId: string
  ) => {
    try {
      ...
      utxos = utxos.filter((utxo: Utxo) => utxo.value === maxValue);
  ...
  getAndBroadcastSignature: async ({
    ...
    try {
      ...
      console.error("Error in getAndBroadcastSignature:", error);
      throw error;
    }
  },
  getSignature: async ({
    ...
    const result = await constructPsbt(address, to, amount, networkId);
    if (!result) return;
    ...
    const keyPair = {
      ...
      sign: async (transactionHash: Buffer) => {
        ...
        signature = await sign(account, contractId, payload, path);
      },
    };
    ...
    try {
      ...
      await Promise.all(
        utxos.map(async (_: Utxo, index: number) => {
          try {
...
async function fetchTransaction(transactionId: string, networkId: string) {
  const bitcoinRpc = `https://blockstream.info/${
    networkId === "testnet" ? "testnet" : ""
  }/api`;
