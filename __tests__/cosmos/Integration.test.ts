...
import type { CosmosNetworkIds } from '../../src/chain-adapters/Cosmos/types'
...
describe('Cosmos MPC Integration', () => {
  let cosmos: Cosmos
  let contract: ChainSignatureContract
  const chainId: CosmosNetworkIds = 'cosmoshub-4' // Cosmos Hub mainnet

  beforeAll(async () => {
    // Get the MPC contract instance from NEAR testnet
    contract = await getNearChainSignatureContract()

    // Initialize the Cosmos adapter with:
    // - Cosmos Hub mainnet for blockchain operations
  ...
  itif('derives Cosmos address from MPC public key on NEAR', async () => {
    ...
    // Use a real derivation path - secp256k1 for Cosmos
    ...
    const { address, publicKey } = await cosmos.deriveAddressAndPublicKey(
      predecessor,
      path
    )
    ...
    const cosmosAddressRegex = /^cosmos1[a-z0-9]{38}$/
    expect(address).toMatch(cosmosAddressRegex)

    // Optional: Try to validate the address by fetching its balance
    try {
      const { balance, decimals } = await cosmos.getBalance(address)
  ...
  itif(
    'can prepare and finalize a Cosmos transaction with MPC signatures from NEAR',
    async () => {
      try {
        // Use a real Cosmos address and public key
        const fromAddress =
          process.env.COSMOS_FROM_ADDRESS ||
          'cosmos1xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx'
        const toAddress =
          process.env.COSMOS_TO_ADDRESS ||
          'cosmos1yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy'
        const publicKey = process.env.COSMOS_PUBLIC_KEY || '02' + 'a'.repeat(64) // Mock compressed pubkey if not provided
        ...
        const amount = '1000' // A very small amount (in uatom) for testing
        ...
        const txRequest = {
          ...
          publicKey,
          messages: [msgSend],
          memo: 'ChainSig Cosmos integration test',
          gas: 200000,
        }
        ...
        let signedTx: string
        try {
          signedTx = cosmos.finalizeTransactionSigning({
            transaction,
